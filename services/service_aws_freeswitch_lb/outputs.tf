...
ssh -i ${var.prefix}-key-${random_string.tag.result}.pem ubuntu@${module.bastion.public_dns}

2) SSH to the FreeSWITCH instance:
ssh -i ${var.prefix}-key-${random_string.tag.result}.pem ubuntu@<FreeSWITCH_Instance_Private_IP> -o "ProxyCommand ssh -W %h:%p -i ${var.prefix}-key-${random_string.tag.result}.pem ubuntu@${module.bastion.public_dns}"

Replace <FreeSWITCH_Instance_Private_IP> with the private IP address of the FreeSWITCH instance.
e.g.:
ssh -i ${var.prefix}-key-${random_string.tag.result}.pem ubuntu@${module.freeswitch.freeswitch_private_ips[0]} -o "ProxyCommand ssh -W %h:%p -i ${var.prefix}-key-${random_string.tag.result}.pem ubuntu@${module.bastion.public_dns}"
