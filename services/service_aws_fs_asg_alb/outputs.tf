...
EOT
  ...
  service_config = <<EOT
{
  "alb_domain": "${module.fs_alb.alb_dns_name}"
}
EOT
...
ALB domain name:
$(module.fs_alb.alb_dns_name)

Due to the nature of the Auto Scaling Group (ASG), the public and private IPs may change over time.
To update these IPs, please run the "deploy refresh" command again

Public:
${join("\n", module.freeswitch_asg.public_ips[*])}


Private:
${join("\n", module.freeswitch_asg.private_ips[*])}


***SSH Instructions***

1) SSH to the FreeSWITCH instance by FreeSwitch's IP:
...
e.g.:
ssh -i ${var.prefix}-key-${random_string.tag.result}.pem ubuntu@${module.freeswitch_asg.public_ips[0]}

2) SSH to Freeswitch instance by ALB
ssh -i ${var.prefix}-key-${random_string.tag.result}.pem ubuntu@${module.fs_alb.alb_dns_name}
